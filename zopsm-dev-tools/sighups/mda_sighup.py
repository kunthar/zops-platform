...
# Find mda service containers and add them to known docker_container_names
mda_index, mda_status = consul_client.health.service(service='mda')
mda_container_names = reduce(reducer , mda_status, [])
try:
    docker_container_names.extend(mda_container_names)
except TypeError:
    logging.critical('Consul Watch: All MDA containers are in critical state.')
    pass
