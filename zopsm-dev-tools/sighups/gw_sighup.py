...
# Find gw service containers and add them to known docker_container_names
gw_index, gw_status = consul_client.health.service(service='gw')
gw_container_names = reduce(reducer , gw_status, [])
try:
    docker_container_names.extend(gw_container_names)
except TypeError:
    logging.critical('Consul Watch: All GATEWAY containers are in critical state.')
    pass
