...
# Find auth service containers and add them to known docker_container_names
auth_index, auth_status = consul_client.health.service(service='auth')
auth_container_names = reduce(reducer , auth_status, [])
try:
    docker_container_names.extend(auth_container_names)
except TypeError:
    logging.critical('Consul Watch: All AUTH containers are in critical state.')
    pass
